...
export interface DetachedForkRequest {
  ...
  /** Whether the side branch ran with the core tool set enabled (/tools). */
 ...
 * live session-owned SessionManager is never mutated.
 */
export function materializeDetachedFork(request: DetachedForkRequest): DetachedForkResult {
  validatePromotableMessages(request.messages, request.toolsEnabled === true);
  ...
  // same-filesystem hard link. A second terminal can therefore never discover
  // a partially appended BTW2 transcript.
  const stagingDir = mkdtempSync(join(request.sourceSessionDir, ".pi-btw2-"));
  let target: SessionManager | undefined;
  let stagedPath: string | undefined;
  let publishedPath: string | undefined;
  let publishedByUs = false;
  try {
    // Preserve the live manager's effective cwd. pi can resume a session with
    // a cwd override that intentionally differs from its stored header, and
    // detached branches must inherit the effective workspace.
    ...
    // and thinking level that generated the BTW2 transcript.
    ...
    // pi writes the staged file when the first assistant message is appended
    // (or immediately in createBranchedSession when the parent branch already
    // contains one); by this point every entry — header, branch path,
    // model/thinking changes, promoted turns, session name — is on disk. The
    // new file remains a sleeping resume target until another terminal opens
    // it; pi tracks no per-file active marker.
