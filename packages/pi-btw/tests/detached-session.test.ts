...
function createSource(root: string): {
  ...
  assert.ok(readFileSync(sourcePath, "utf8").length > 0, "pi must persist the parent fixture");
  return {
    cwd,
    sessionsDir,
    manager,
...
test("materializeDetachedFork atomically publishes a resumable sleeping branch", () => {
  const root = mkdtempSync(join(tmpdir(), "pi-btw-test-"));
  try {
    ...
    assert.equal(result.sessionPath, join(source.sessionsDir, basename(result.sessionPath)));
    assert.ok(readFileSync(result.sessionPath, "utf8").length > 0);
    assert.equal(readFileSync(source.sourcePath, "utf8"), source.sourceBytes, "source file must remain byte-identical");
    assert.equal(
      readdirSync(source.sessionsDir).filter((name) => name.startsWith(".pi-btw2-")).length,
      0,
    ...
    const latestInfo = entries.findLast((entry) => entry.type === "session_info");
    assert.equal(
      latestInfo?.type === "session_info" ? latestInfo.name : undefined,
      "Detached BTW2",
    );
