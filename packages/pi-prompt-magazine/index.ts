 ...
 *
 * This is the pi port of the Prime Agent `pa-prompt-magazine` package: the
 * same commands, capture marker, and op-log persistence, fitted to the pi
 * extension API. pi's `input` event maps 1:1 to Prime's (source
 * "interactive" for editor submissions; RPC/script/extension inputs pass).
 */
...
const CUSTOM_TYPE = "pi-prompt-magazine";
...
export default function promptMagazine(pi: ExtensionAPI): void {
  ...
  /** Interactive queue browser built on select/confirm/editor dialogs. */
