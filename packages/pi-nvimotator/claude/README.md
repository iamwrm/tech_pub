...
```bash
cd /absolute/path/to/piagent-config/packages/pi-nvimotator && npm ci && npm run build
```
...
```text
/plugin marketplace add /absolute/path/to/piagent-config
...
```text
/plugin marketplace add iamwrm/piagent-config
/plugin install nvimotator@nvimotator
```
...
```bash
claude --plugin-dir /absolute/path/to/piagent-config/packages/pi-nvimotator/claude
```
