...
# macOS TMPDIR ends in "/" and /var is a symlink to /private/var, while Neovim reports its
# canonical getcwd(); resolve the fixture directory once so the attach assertion compares like with like.
...
find_registry_file() {
  find "$REGISTRY" -maxdepth 1 -type f -name '[1-9]*.json' -print | head -1
}
