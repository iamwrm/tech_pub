import { collectingIo, withStoreRoot } from "./cli-helpers.ts";
import assert from "node:assert/strict";
import { chmod, readFile, writeFile } from "node:fs/promises";
import { join } from "node:path";
import test from "node:test";
import { snapshotFromAssistantText } from "../src/assistant-message.ts";
import { CliExitError, runFileStoreCli } from "../src/file-cli.ts";
import { FileStore } from "../src/file-store.ts";
import { markSlotSent } from "./sent-slot.ts";


const withStore = <T>(fn: (store: FileStore, root: string) => Promise<T>) =>
  withStoreRoot((root) => fn(new FileStore(root), root));
