...
Prime Agent extension that turns the single-slot prompt stash into a **queue of stashed drafts** — a "magazine" (弹匣). Type a long draft, decide you don't want to send it, and stash it with two semicolons. Load several drafts, then browse the queue and restore any one of them into the prompt editor.
...
```bash
prime-agent package install ./packages/pa-prompt-magazine   # from this repo
# or
prime-agent package install /absolute/path/to/pa-prompt-magazine
```

Then start a new session (or `/reload`). Remove with `prime-agent package remove pa-prompt-magazine`.
...
- **No keyboard shortcuts**: prime-agent 0.7.0 does not dispatch extension shortcuts in the standard daemon CLI, so the capture path is the `;;` marker above (the built-in Ctrl+S single-slot stash is unaffected).

## Development

```bash
cd packages/pa-prompt-magazine
npm install        # installs typecheck deps from the published type tarballs
...
- `magazine.ts` — pure FIFO queue model + stash-marker parsing (no prime-agent imports; unit-tested)
