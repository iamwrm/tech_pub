...
Status: **active** (`ren-public-package` 0.9.3; default-on for strict Codex subscription and Fluxion standard-Responses allowlists)
...
- Dogfood the 0.9.3 protocol-specific Codex and Fluxion adapters, bounded
  transient retry, canonical replay, exact prepared-payload capture, and
  transcript UI in ordinary long sessions.
...
- Direct official `openai/*` API support cannot be authenticated in the current
  model catalog. Enterprise mirrors remain unsupported until each exact
  model/route passes artifact creation and stateless semantic replay.
- Authenticated matrix expansion consumes quota and requires approval.

### BACKLOG

- Extend the standard adapter to additional enterprise mirrors only after exact
  tuple qualification; HTTP 200 or another model on the host is insufficient.
...
Long Codex and qualified enterprise-mirror sessions should use native Responses
compaction while respecting their different wire protocols:
...
- Intercept Pi manual, threshold, and overflow boundaries through
  `session_before_compact` for strictly allowlisted Codex and Fluxion Responses
  models.
- Capture the exact logical provider payload after Pi's complete
  provider-request hook chain, including downstream prompt/tool mutations and
  provider serialization.
- For Codex, send that exact payload with only post-snapshot history and a final
  `{ "type": "compaction_trigger" }`; keep normal turns and compaction on the
  same SSE/cache lane.
- For standard Responses, project the prepared payload to `model`, complete
  `input`, optional `instructions`, and `prompt_cache_key`, append converted
  post-snapshot history, and POST JSON to `/responses/compact` with ordinary
  Bearer authentication—never Codex trigger/account/feature headers.
...
- Retry only bounded transient transport/5xx/explicit-overload failures; never
  ...
  or abort failures.
- Persist Codex's canonicalized recent-user window plus artifact; for standard
  Responses, preserve the complete provider-returned canonical `output` array
  unchanged. Both paths require exactly one artifact and bounded safe storage.
...
- Recursively replay the opaque window plus exact branch tail. Codex permits
  model changes on the same endpoint; standard mirror artifacts are bound to the
  exact model until cross-model replay is separately qualified.
...
threshold logic. It registers stream decorators around Pi's own Codex and
standard Responses implementations so it can observe the final logical payload
outside the exception-swallowing extension hook runner. Registrations for
`fluxion-gpt` and `fluxion-grok` contain no model list, so Pi preserves the
existing `models.json` catalog and authentication. Codex deliberately selects
SSE for normal turns so inference and manual compaction share one cache lane;
standard Fluxion turns keep Pi's ordinary Responses transport.
...
The current 0.9.x schema is intentionally direct and identifies the wire
adapter plus its exact replay owner:
...
Older 0.9.0–0.9.2 Codex records without `adapter` are read as
`codex-trigger-sse`; no standard record is inferred.

Codex `replacementHistory` ends in one canonicalized `compaction` item; optional
ID and safe turn metadata survive while unknown wire fields are discarded.
Standard Responses instead stores the complete returned `output` array exactly
as received—including the `compaction` versus `compaction_summary` spelling,
order, IDs, and vendor fields—after size, artifact-cardinality, and credential
screening. The sole artifact must be final.
...
At Codex compaction, the request is:
...
The standard adapter instead projects the final payload to the unary compact
contract—`model`, full `input`, optional `instructions`, and
`prompt_cache_key`—then appends converted branch tail. It sends neither
create-only options nor `compaction_trigger`.

Tail conversion selects Pi's matching implementation:
`openAICodexResponsesApi().streamSimple` for Codex or
`openAIResponsesApi().streamSimple` for standard Responses, using a synthetic
credential, loopback base URL, and a capture that throws before transport. The
converter receives the full registered tool catalog so historical
...
After a valid Codex response, the local replacement window is:
...
A standard Responses server returns the replacement window itself:

```text
provider canonical output items, unchanged + exactly one final opaque artifact
```

Standard output is not rebuilt from local history. It is accepted only under the
same 8 MiB persistence bound and rejected if it contains credential/header
fields, secret-shaped strings, or remote URL fields. AGENTS/environment/skill/
hook/internal context envelopes are not duplicated as explicit retained user
messages in the Codex window. Pi-serialized local image data URLs are
...
### 4.2 Enterprise-mirror protocol boundary

An authenticated Fluxion probe showed that server compaction is a
**model/backend capability, not a host-wide promise**:

```text
POST /v1/responses/compact
  GPT-5.5      -> 200 + one compaction_summary
  GPT-5.6 Sol  -> 200 + one compaction_summary -> replay recalled marker
  Grok 4.5     -> 200 + one compaction         -> replay recalled marker
  Kimi K3      -> 200 + ordinary generation, no artifact (unsupported)
  Claude       -> Anthropic Messages; this endpoint does not apply
```

HTTP 200 alone is therefore not proof. Acceptance requires exactly one opaque
artifact, then a stateless `/responses` replay of the **canonical compact output
as-is** plus a new user turn. Fluxion GPT returned `message +
compaction_summary`; Grok returned `compaction`. The adapter must preserve these
provider-produced windows rather than forcing them into the Codex-specific
`recent user items + compaction` schema.

Version 0.9.3 activates separate adapters rather than broadening the Codex
allowlist:

```text
openai-codex
  -> POST /backend-api/codex/responses
  -> compaction_trigger + named SSE
  -> Codex JWT/account headers

fluxion-gpt/{gpt-5.5,gpt-5.6-sol}, fluxion-grok/grok-4.5
  -> POST /v1/responses/compact
  -> unary JSON; preserve canonical output as-is
  -> ordinary Bearer key; no Codex headers

all other mirror tuples -> ordinary Pi compaction
```

The standard adapter is exact-model-bound, while Codex remains same-endpoint
cross-model compatible. A 200 response with no artifact—observed on Kimi K3—is
a permanent validation failure, not support. See
[IV-0004](./IV-0004-enterprise-mirror.md) for the reusable qualification matrix.
No credential values, authorization headers, full response bodies, prompts, or
opaque artifacts belong in either IV; record only models, status, item types,
bounded sizes, and semantic pass/fail results.
...
- No decoding, inspection, or editing of provider ciphertext/opaque state.
...
- No provider reimplementation, custom WebSocket manager, `store: true`,
  ...
  `previous_response_id` state. Protocol-specific stream decorators delegate to
  Pi's published Codex/Responses APIs; only the Codex path forces SSE.
- No branch-summary replacement for `/tree`.
- No direct OpenAI/Azure or additional proxy support without authenticated
  evidence and an exact protocol/model adapter; compatible endpoint names alone
  are insufficient.
...
Observed offline for 0.9.3 on Pi 0.83.0:

- strict typecheck passes;
- all 134 unit tests pass, including 40 focused `0017` unit tests;
- all 4 integration tests pass, including real Pi/model-config composition;
- real model-list composition preserves Fluxion GPT/Grok/Kimi catalogs;
- `git diff --check` and npm 0.9.3 package dry-run pass.
...
Focused 0.9.3 coverage includes:

- strict Codex and Fluxion endpoint/model allowlists, persisted-history, SSE/JSON,
  replay, fallback-ownership, tool-tail, exact-payload, bounded-retry,
  retained-image, and UI coverage;
- standard unary request projection, ordinary Bearer headers without Codex
  fields, complete canonical-output preservation, GPT/Grok artifact variants,
  Kimi-style 200-without-artifact rejection, exact-model binding, recursive
  latest-checkpoint replay, and ownership-safe retry exhaustion;
...
### Authenticated results — Fluxion standard adapters (2026-07-30, 0.9.3)

Fresh disposable Pi 0.83 RPC sessions validated the complete extension path for
all three exact standard-adapter tuples:

```text
Pi normal request -> final payload capture -> JSON /responses/compact
-> Pi compaction JSONL -> next /responses replay -> exact nonce recall
```

- `fluxion-gpt/gpt-5.5`: one native/zero plain checkpoints; four canonical
  output items ending in `compaction_summary`; 1,572-byte artifact; exact recall.
- `fluxion-gpt/gpt-5.6-sol`: one native/zero plain checkpoints; four canonical
  output items ending in `compaction_summary`; 1,208-byte artifact; exact recall;
  a fresh Pi process resumed the checkpoint and recalled exactly again.
- `fluxion-grok/grok-4.5`: one native/zero plain checkpoints; canonical output
  was the single `compaction` item; 6,912-byte artifact; exact recall.

Persisted details used `standard-responses-json`, matched the exact provider/API/
base endpoint/model owner, and contained no credential-shaped keys. No Codex
account/feature headers were used. Evidence files stayed in mode-restricted
`/tmp` directories during validation, were then removed, and were not committed;
the IV records only bounded metadata.
...
- **Provider override conflict:** a later `openai-codex`, `fluxion-gpt`, or
  `fluxion-grok` stream registration can supersede its decorator and remove
  final-payload capture. Re-check installed providers when adding adapters.
...
- TUI, RPC, JSON, and print sessions using the public bundle and supported Codex
  or Fluxion models. Native Codex turns use Pi's SSE transport rather than its
  cached WebSocket transport; Fluxion retains Pi's ordinary Responses stream.
