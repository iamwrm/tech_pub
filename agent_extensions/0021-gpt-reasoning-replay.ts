...
/** Preserve qualified opaque reasoning only while both source and target IDs are GPT. */
export default function gptReasoningReplay(pi: ExtensionAPI): void {
	pi.on("context", (event, ctx) => {
		if (!featureEnabled(process.env[GPT_REASONING_REPLAY_ENV]) || !ctx.model) return undefined;
		const messages = prepareGptReasoningReplay(event.messages, ctx.model);
